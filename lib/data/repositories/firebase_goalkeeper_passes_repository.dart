...
class FirebaseGoalkeeperPassesRepository implements GoalkeeperPassesRepository {
  ...
  Future<List<GoalkeeperPassModel>> getPassesByUser(String userId) async {
    try {
      ...
      final cachedPasses =
          await _cacheManager.get<List<GoalkeeperPassModel>>(cacheKey);
  ...
  Future<GoalkeeperPassModel?> getPassById(String id) async {
    try {
      ...
      // Si no hay caché, consultar a Firestore
      final doc = await _firestore.collection(_passesCollection).doc(id).get();
  ...
  Future<GoalkeeperPassModel> createPass(GoalkeeperPassModel pass) async {
    try {
      ...
      if (pass.matchId != null && !await _verifyPremiumForMatch(pass.userId)) {
        throw Exception(
            'Solo los usuarios premium pueden asociar saques a partidos');
      }
      ...
      FirebaseCrashlytics.instance
          .recordError(e, StackTrace.current, reason: 'Error al crear saque');
  ...
  Future<GoalkeeperPassModel> updatePass(GoalkeeperPassModel pass) async {
    try {
      ...
      await _firestore.collection(_passesCollection).doc(pass.id).update({
        ...pass.toMap(),
      ...
      final updatedDoc =
          await _firestore.collection(_passesCollection).doc(pass.id).get();

      // Invalidar caché relevante
      await _invalidateUserCache(pass.userId);
      if (currentPass.matchId != null) {
        await _cacheManager
            .remove('$_matchPassesCachePrefix${currentPass.matchId}');
      }
  ...
  Future<void> deletePass(String id) async {
    try {
      ...
      if (pass != null) {
        await _firestore.collection(_passesCollection).doc(id).delete();
  ...
  Future<List<GoalkeeperPassModel>> getPassesByResult(
      String userId, String result) async {
    try {
      // Intentar obtener desde caché
  ...
  Future<List<GoalkeeperPassModel>> getPassesByDateRange(
    ...
    try {
      ...
      final cacheKey =
          '${_userPassesCachePrefix}${userId}_${startDate.millisecondsSinceEpoch}_${endDate.millisecondsSinceEpoch}';
      final cachedPasses =
          await _cacheManager.get<List<GoalkeeperPassModel>>(cacheKey);
  ...
  Stream<List<GoalkeeperPassModel>> watchUserPasses(String userId) {
    return _firestore
      ...
      _cacheManager.set('$_userPassesCachePrefix$userId', passes,
  ...
  Stream<List<GoalkeeperPassModel>> watchMatchPasses(String matchId) {
    return _firestore
      ...
      // Actualizar caché cada vez que hay cambios
      _cacheManager.set('$_matchPassesCachePrefix$matchId', passes,
          duration: _cacheDuration);
  ...
  Future<void> _invalidateUserCache(String userId) async {
    ...
    for (final type in [
      ...
      await _cacheManager
          .remove('${_userPassesCachePrefix}${userId}_type_$type');
    }

    // Resultados
    await _cacheManager.remove(
        '${_userPassesCachePrefix}${userId}_result_${GoalkeeperPassModel.RESULT_SUCCESSFUL}');
    await _cacheManager.remove(
        '${_userPassesCachePrefix}${userId}_result_${GoalkeeperPassModel.RESULT_FAILED}');
  }
}
